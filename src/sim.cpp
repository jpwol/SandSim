...
void Sim::init(){
    if(!SDL_Init(SDL_INIT_VIDEO)){
        window = SDL_CreateWindow("Sand Sim", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 1280, 720, 0);
        if(window == NULL){
...
void Sim::update(){
    ...
    //std::cout << mouseX/matrix.particleSize << ", " << mouseY/matrix.particleSize << "\n";
    if(mouseState == 1){
    ...
    for(int x = 0; x < matrix.worldWidth; x++){
        for(int y = 0; y < matrix.worldHeight; y++){
            if(matrix.matrix[x][y] != nullptr){
                SDL_SetRenderDrawColor(renderer, matrix.matrix[x][y]->r, matrix.matrix[x][y]->g, matrix.matrix[x][y]->b, 255);
                SDL_Rect rect;
                rect = {x*matrix.particleSize, y*matrix.particleSize, matrix.particleSize, matrix.particleSize};
